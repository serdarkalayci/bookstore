...
// ExtractBookInfoRequestPayload extracts bookInfo data from the request body
// Returns bookInfoRequestDTO model if found, error otherwise
func ExtractBookInfoRequestPayload(r *http.Request) (bookInfo *dto.BookInfoRequestDTO, e error) {
	payload, e := readPayload(r)
	if e != nil {
		return
	}
	err := json.Unmarshal(payload, &bookInfo)
	if err != nil {
